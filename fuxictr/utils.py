...
def set_logger(params):
    ...
    # 优先使用 exp_id，如果不存在则使用 model_id
    exp_id = params.get('exp_id', model_id)

    # log 文件放在 checkpoints/{dataset_id}/{exp_id}/ 目录内
    log_dir = os.path.join(params.get('model_root', './checkpoints'), dataset_id, exp_id)
