...
```bash
python run_expid.py --config ./config/ --expid APG_SharedBottom_test --mode inference --sweep
```
...
3. Output a directory (e.g., `APG_SharedBottom_test_inference_result`) containing partitioned Parquet files (`part_*.parquet`).
