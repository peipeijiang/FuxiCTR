...
```bash
python run_expid.py --config ./config/ --expid M3oE_test --mode train
```

### Inference

```bash
python run_expid.py --config ./config/ --expid M3oE_test --mode inference
```
...
```bash
python run_expid.py --config ./config/ --expid M3oE_test --mode inference --sweep
```
...
3. Output a directory (e.g., `M3oE_test_inference_result`) containing partitioned Parquet files (`part_*.parquet`).
...
- **Experiment ID** (e.g., `M3oE_test`): Specific configurations for an experiment.
    ...
    - **Model-specific parameters**: Parameters specific to the model architecture (e.g., `num_domains`, `expert_num`).
