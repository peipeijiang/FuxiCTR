...
```bash
python run_expid.py --config ./config/ --expid APG_AITM_test --mode train
```

### Inference

```bash
python run_expid.py --config ./config/ --expid APG_AITM_test --mode inference
```
...
```bash
python run_expid.py --config ./config/ --expid APG_AITM_test --mode inference --sweep
```
...
3. Output a directory (e.g., `APG_AITM_test_inference_result`) containing partitioned Parquet files (`part_*.parquet`).
...
- **Experiment ID** (e.g., `APG_AITM_test`): Specific configurations for an experiment.
